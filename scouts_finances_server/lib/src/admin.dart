...
class AdminEndpoint extends Endpoint {
  Future<void> resetDb(Session session) async {
    ...
    final insertedParents = await Parent.db.insert(session, parents);
    ...
    final insertedChildren = await Child.db.insert(session, children);
    ...
    for (final group in insertedGroups) {
      ...
      for (var i = 0; i < numEvents; i++) {
        ...
        final cost = 5 + Random().nextInt(2000); // Cost between 5 and 25 pounds
        // Some time in the next year
        final date = DateTime.now().add(Duration(days: Random().nextInt(365)));
        events.add(Event(
          name: eventName,
          cost: cost,
          date: date,
    ...
    final insertedEvents = await Event.db.insert(session, events);
